...
2. Clicar em **Cadastrar novo chip**;
3. Informar nome, código e quantidade de pinos;
4. Marcar os pinos como entrada ou saída;
5. Preencher os nomes opcionais dos pinos;
6. Salvar o chip.
...
1. Na tela inicial, clicar em **Testes** no chip desejado;
2. Definir os valores esperados para entradas e saídas;
3. Usar `0`, `1` ou `Não incluir`;
4. Adicionar quantos testes forem necessários;
5. Salvar os testes.

O firmware aceita até **10 testes por chip**.
...
2. Garantir que a porta serial configurada seja `COM5`;
3. Na interface web, clicar em **Enviar chip**;
4. O Flask envia o protocolo serial;
5. O Arduino salva o chip na EEPROM;
6. A interface informa se o envio foi confirmado.

---

### 4. Identificar um Chip

1. Inserir o chip no soquete;
2. Pressionar o botão físico;
3. O Arduino percorre os chips salvos;
4. Executa os testes cadastrados;
5. Exibe o resultado no display TFT.
...
- O sistema trabalha com chips de **14 ou 16 pinos**.
- O firmware aceita até **10 testes por chip**.
- A EEPROM comporta até **14 chips** na estrutura atual.
- A porta serial está configurada como `COM5`.
- O envio de um chip para o Arduino grava um novo cadastro na EEPROM.
- Para limpar os cadastros do Arduino, deve-se usar o comando `Limpar`.

---

## Resumo Final

O projeto utiliza uma arquitetura dividida entre uma aplicação web e um sistema embarcado.

A interface web facilita o cadastro dos chips e testes. O Flask organiza esses dados, mantém uma cópia em JSON e envia o cadastro ao Arduino por Serial USB. O Arduino interpreta o protocolo recebido, salva os dados na EEPROM e usa essas informações para identificar fisicamente chips da família 74xx.

Essa separação permite que o sistema tenha uma interface amigável para configuração, mas continue funcionando de forma independente no Arduino durante a identificação dos componentes.
